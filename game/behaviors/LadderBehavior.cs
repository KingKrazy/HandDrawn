...
{
%this.playerinTrigger = 1;
%theirobject.rotation = 0;
    // Record that we have an active ladder
    %theirObject.LadderObject = %this.Owner;
    %theirObject.LadderActive = false;
    %this.schedule(3000, "generateArrow");
}

function LadderBehavior::generateArrow(%this)
{
if(!isObject(%this.UpArrow) && %this.PlayerinTrigger && !$game::player.linearvelocity.y > 0)
{
    %posX = %this.Owner.Position.X;
    %posY = $game::player.position.Y;
    %aPos = %posX SPC %posY;


   %this.UpArrow = new t2dAnimatedSprite()
    {
      SceneGraph = Scenewindow2D.getSceneGraph();
      animationName = "ClimbArrowAnimation";
...
echo("Arrow created at " @ %aPos @ ".");
}
}

function LadderBehavior::onLeave( %this, %theirObject )
{
%this.PlayerinTrigger = 0;
%this.upArrow.safeDelete();
Warn("Arrow at " @ %aPos @ "was destroyed.");

%theirObject.getAnimationPuppet().setRotation(0);
