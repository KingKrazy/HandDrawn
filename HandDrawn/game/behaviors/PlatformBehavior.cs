...
function platformBehavior::onEnter(%this, %theirobject)
{
//Um... Whatever :P
}

function platformBehavior::onLeave(%this, %theirobject)
{
    if(%theirObject.onWall)
    {
    %theirObject.setAnimationState("fall");
    }
}

function platformBehavior::onCollision(%this, %theirobject)
{
    if ( %theirObject.isMemberOfClass( "pskProjectile" ) )
    {
    %theirObject.triggerImpact();
    echo("BAM!");
    }
}
...
%puppet = %actor.getAnimationPuppet();

%actor.getAnimationPuppet().setRotation(%this.owner.getRotation());


if(%puppet.rotation > 45)
{
%actor.getAnimationPuppet().setRotation(0);
}
