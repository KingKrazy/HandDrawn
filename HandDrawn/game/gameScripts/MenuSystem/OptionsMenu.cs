...
function _loadVideoSettings(%xmlFile)
{
	...
	%SmartStream      = %xmlFile.readField( "SmartStream" );
	%SmartHUD      = %xmlFile.readField( "SmartHUD" );
	%StreamDist = %xmlFile.readField( "StreamDist" );
	
    %screenshot = %xmlFile.readField( "Screenshot" );
	if (%screenshot $= "")
		%screenshot = %xmlFile.readField( "screenShot" );
	
	_setVideoPreferences(%mode, %driver, %resolution, %depth, %vSync, %screenshot, %SmartStream, %SmartHUD, %streamDist);
}

function _setVideoPreferences(%mode, %driver, %resolution, %depth, %vSync, %screenshot, %SmartStream, %SmartHUD, %streamDist)
{
	...
	if (%smartStream !$= "") _setPreference("VideoSettings::SmartStream", 	%smartStream);
	if (%smartHUD !$= "") _setPreference("VideoSettings::SmartHud", 	%smartHud);
	if (%streamDist !$= "") _setPreference("VideoSettings::StreamDist", 	%treamDist);
}
...
function _applyPreferenceVideoSettings()
{
	...
	%smartStream = _getPreference("VideoSettings::SmartStream");
    %smartHud = _getPreference("VideoSettings::SmartHud");
    %streamDist = _getPreference("VideoSettings::StreamDist");

	// Apply the settings
	_applyVideoSettings(%mode, %driver, %resolution, %depth, %vSync, %screenshot, %smartStream, %smartHUD, %streamDist);
}

function _applyVideoSettings(%mode, %driver, %resolution, %depth, %vSync, %screenshot, %smartStream, %smartHUD, %streamDist)
{
